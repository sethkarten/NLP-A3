...
class GlobalAttention(nn.Module):
    ...
    def score(self, Q, K):
        """
               (BxT'xd) (BxTxd)  ------->  (BxT'xT)
        """
        # print(self.dim, Q.shape, self.linear_in(Q).shape, K.transpose(2,1).shape)
        return torch.bmm(self.linear_in(Q), K.transpose(2, 1))
        # print(torch.allclose(K.reshape(K.shape[0], self.dim, -1), K.transpose(0, 2,1)))
        # return torch.bmm(self.linear_in(Q), K.reshape(K.shape[0], self.dim, -1))
